...
# -------------------------------
# 1. Definición del modelo StudentCNN
# -------------------------------
...
class StudentCNN(nn.Module):
    def __init__(self, num_classes=4):
        ...
        # Calcular automáticamente el tamaño de entrada de la capa lineal
        dummy_input = torch.zeros(1, 3, 224, 224)  # batch=1, canal=3, 224x224
        dummy_output = self.features_student(dummy_input)
        num_features = dummy_output.view(1, -1).size(1)  # aplanado
...
def train_student_with_early_stopping(student_model, teacher_model, train_loader, val_loader,
    ...
    for epoch in range(epochs):
        ...
        torch.save(student_model.state_dict(), "student_model_cnn_3capa.pth")
...
if __name__ == "__main__":
    ...
    if os.path.exists("student_model_cnn_3capa.pth"):
        size_mb = os.path.getsize("student_model_cnn_3capa.pth") / (1024 * 1024)
    ...
    performance_assessment_student(student_model, val_loader, model_path_student="student_model_cnn_3capa.pth")
